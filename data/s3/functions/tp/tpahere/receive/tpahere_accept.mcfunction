...
## 存在
execute if score #exist tmp matches 1 run function s3:tp/tpahere/receive/func/exe_tp
execute if score #exist tmp matches 1 run function s3:tp/tpahere/receive/tellraw/done/tp
scoreboard players operation #uid tmp = @s uid
scoreboard players operation # tmp = @s tpahere.send.uid
execute if score #exist tmp matches 1 as @a if score @s uid = # tmp run function s3:tp/tpahere/send/tellraw/done/tp
