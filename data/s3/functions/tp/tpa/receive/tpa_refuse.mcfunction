...
## 存在
execute if score #exist tmp matches 1 run function s3:tp/tpa/receive/tellraw/done/refuse
scoreboard players operation #uid tmp = @s uid
scoreboard players operation # tmp = @s tpa.send.uid
execute if score #exist tmp matches 1 as @a if score @s uid = # tmp run function s3:tp/tpa/send/tellraw/done/refuse
